...
class Resting(smach.State):
    def __init__(self):
        ...
        self.takeoff_pub = rospy.Publisher(self.tello_ns + 'takeoff', Empty, queue_size=1);

        rospy.Subscriber('land', Empty, callback=self.myTelloLandCb, queue_size=1);
...
class Hovering(smach.State):
    def __init__(self):
        ...
        rospy.Subscriber('takeoff', Empty, callback=self.myTelloTakeoffCb, queue_size=1);
        rospy.Subscriber('emergency', Empty, callback=self.myTelloEMCb, queue_size=1);
        rospy.Subscriber('/EOT', Empty, callback=self.myTelloEOTCb, queue_size=1);

        # self.SOT_pub = rospy.Publisher('/SOT', Empty, queue_size=1);
        self.land_pub =  rospy.Publisher(self.tello_ns + 'land', Empty, queue_size=1);
...
class Testing(smach.State):
    def __init__(self):
        ...
        # publisher
        self.SOT_pub = rospy.Publisher(self.tello_ns + 'SOT', Empty, queue_size=1);
